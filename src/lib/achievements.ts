...
// Derived from the curriculum so badge copy can't drift from the real lesson
// counts, and so stale ids from retired lessons don't count toward All-Star.
const INVESTING_IDS = INVESTING_LESSONS.map((l) => l.id);
const ALL_LESSON_IDS = TRACKS.flatMap((t) => t.lessons.map((l) => l.id));
...
export const ACHIEVEMENTS: Achievement[] = [
  ...
  { id: "portfolio", icon: "⊞", name: "Portfolio Mgr",  desc: `Finish all ${INVESTING_IDS.length} Investing lessons`, total: INVESTING_IDS.length, progress: (ids) => count(ids, INVESTING_IDS) },
  ...
  { id: "allstar",   icon: "✶", name: "All-Star",       desc: `Complete all ${ALL_LESSON_IDS.length} lessons`, total: ALL_LESSON_IDS.length, progress: (ids) => count(ids, ALL_LESSON_IDS) },
];
